...
import com.synerset.hvaclib.fluids.dryair.FlowOfDryAir;
...
/**
 * PSYCHROMETRICS PROCESS EQUATIONS LIBRARY <p>
 * Set of static methods outputs process result as an array with process heat, core output air parameters
 * (temperature, humidity ratio) and condensate properties. Methods do not create a separate instance of FlowOfMoistAir
 * for performance reasons - each ot these methods may be used in iterative solvers, and we
 * do not want to lose memory or performance for unnecessary object creation. <p>
 * Variable literals have the following meaning: (1) or in - stands for input/inlet air, (2) or out - stands for
 * output/outlet air. <p>
 * <p>
 * PREFERENCE SOURCES: <p>
 * [1] ASHRAE FUNDAMENTALS 2002, CHAPTER 6 <p>
 * [2] Lipska B. "Projektowanie Wentylacji i Klimatyzacji. Podstawy uzdatniania powietrza" Wydawnictwo Politechniki
 * Śląskiej (Gliwice  2014) <p>
 * <p>
...
final class AirCoolingProcedures {
    ...
    public static AirCoolingResult processOfDryCooling(FlowOfHumidAir inletFlow, Power inputHeatQ) {
        ...
        if (inputHeatQ.isZero() || inletFlow.massFlow().isZero()) {
            FlowOfLiquidWater zeroCondensateFlow = createZeroCondensateFlow(inletFlow.temperature());
            return new AirCoolingResult(inletFlow, inputHeatQ.createNewWithValue(0), zeroCondensateFlow, BypassFactor.of(0));
        }

        HumidAir inletHumidAir = inletFlow.fluid();
        double Q_cool = inputHeatQ.getInKiloWatts();
        ...
        FlowOfHumidAir outletFlow = FlowOfHumidAir.ofDryAirMassFlow(outletHumidAir, inletFlow.dryAirMassFlow());
        FlowOfLiquidWater zeroCondensateFlow = createZeroCondensateFlow(outletFlow.temperature());

        return new AirCoolingResult(outletFlow, inputHeatQ, zeroCondensateFlow, BypassFactor.of(0));
    }

    /**
     * Calculates outlet cooling power (heat of process) for dry cooling case based on target outlet temperature. Target temperature must be lower than inlet flow temp for valid cooling case.<p>
    ...
    public static AirCoolingResult processOfDryCooling(FlowOfHumidAir inletFlow, Temperature targetOutTemp) {
        ...
        if (targetOutTemp.isEqualOrGreaterThan(inletFlow.temperature())) {
            return new AirCoolingResult(inletFlow, Power.ofWatts(0), zeroCondensateFlow, BypassFactor.of(0));
        }

        // If target temperature is below dew point temperature it is certain that this is no longer dry cooling
        if (targetOutTemp.isLowerThan(inletFlow.fluid().dewPointTemperature())) {
            return new AirCoolingResult(inletFlow, Power.ofWatts(0), zeroCondensateFlow, BypassFactor.of(0));
        }
        ...
        zeroCondensateFlow = createZeroCondensateFlow(outletFlow.temperature());

        return new AirCoolingResult(outletFlow, requiredHeat, zeroCondensateFlow, BypassFactor.of(0));
    }
    ...
    /**
     * Returns real cooling coil process result as double array, to achieve expected outlet Relative Humidity. Results in the array are organized as following:<p>
    ...
    public static AirCoolingResult processOfRealCooling(FlowOfHumidAir inletFlow, Temperature averageWallTemp, RelativeHumidity targetOutRH) {
        ...
        if (RH_out == RH_in) {
            ...
            return new AirCoolingResult(inletFlow, Power.ofWatts(0.0), flowOfLiquidWater, coilBypassFactor(averageWallTemp, inletHumidAir.temperature(), inletHumidAir.temperature()));
        }
    ...
    /**
     * Returns real cooling coil process result as double array, for provided cooling power. Results in the array are organized as following:<p>
     * result: [heat in (W), outlet air temperature (oC), outlet humidity ratio x (kgWv/kgDa), condensate temperature (oC), condensate mass flow (kg/s)]<p>
    ...
    public static AirCoolingResult processOfRealCooling(FlowOfHumidAir inletFlow, Temperature averageWallTemp, Power coolingPower) {
        ...
        if (Q_cool == 0.0) {
            LiquidWater liquidWater = LiquidWater.of(inletFlow.temperature());
            FlowOfLiquidWater flowOfLiquidWater = FlowOfLiquidWater.of(liquidWater, MassFlow.ofKilogramsPerSecond(0.0));
            new AirCoolingResult(inletFlow, coolingPower, flowOfLiquidWater,
    ...
    private static FlowOfLiquidWater createZeroCondensateFlow(Temperature condensateTemperature) {
        LiquidWater condensate = LiquidWater.of(condensateTemperature);
        return FlowOfLiquidWater.of(condensate, MassFlow.ofKilogramsPerSecond(0));
    }
