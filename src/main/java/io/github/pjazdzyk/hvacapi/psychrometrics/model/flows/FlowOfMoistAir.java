...
import io.github.pjazdzyk.hvacapi.psychrometrics.exceptions.FlowArgumentException;
...
import io.github.pjazdzyk.hvacapi.psychrometrics.physics.PhysicsOfFlow;
...
public class FlowOfMoistAir implements Flow, Serializable, Cloneable {
    ...
    public FlowOfMoistAir(double flowRate) {
        this(Defaults.DEF_FLOW_NAME, flowRate, TypeOfAirFlow.MA_MASS_FLOW, new MoistAir());
    ...
    public void updateFlows() {
        ...
        switch (lockedFlowType) {
            case MA_MASS_FLOW -> {
                volFlowMa = PhysicsOfFlow.calcVolFlowFromMassFlow(moistAir.getRho(), massFlowMa);
                massFlowDa = PhysicsOfFlow.calcDaMassFlowFromMaMassFlow(moistAir.getX(), massFlowMa);
                volFlowDa = PhysicsOfFlow.calcDaVolFlowFromDaMassFlow(moistAir.getRho_Da(), massFlowDa);
            }
            case MA_VOL_FLOW -> {
                massFlowMa = PhysicsOfFlow.calcMassFlowFromVolFlow(moistAir.getRho(), volFlowMa);
                massFlowDa = PhysicsOfFlow.calcDaMassFlowFromMaMassFlow(moistAir.getX(), massFlowMa);
                volFlowDa = PhysicsOfFlow.calcDaVolFlowFromDaMassFlow(moistAir.getRho_Da(), massFlowDa);
            }
            case DA_MASS_FLOW -> {
                massFlowMa = PhysicsOfFlow.calcMaMassFlowFromDaMassFlow(moistAir.getX(), massFlowDa);
                volFlowMa = PhysicsOfFlow.calcVolFlowFromMassFlow(moistAir.getRho(), massFlowMa);
                volFlowDa = PhysicsOfFlow.calcDaVolFlowFromDaMassFlow(moistAir.getRho_Da(), massFlowDa);
            }
            case DA_VOL_FLOW -> {
                massFlowDa = PhysicsOfFlow.calcDaMassFlowFromDaVolFlow(moistAir.getRho_Da(), volFlowDa);
                massFlowMa = PhysicsOfFlow.calcMaMassFlowFromDaMassFlow(moistAir.getX(), massFlowDa);
                volFlowMa = PhysicsOfFlow.calcVolFlowFromMassFlow(moistAir.getRho(), massFlowMa);
            }
        }
    ...
    public double getMinFlow(TypeOfAirFlow typeOfFlow) {
        ...
        switch (typeOfFlow) {
            case DA_VOL_FLOW -> {
                return PhysicsOfFlow.calcDaVolFlowFromDaMassFlow(moistAir.getRho_Da(), minMassFlowDa);
            }
            case MA_VOL_FLOW -> {
                return PhysicsOfFlow.calcMaVolFlowFromDaMassFlow(moistAir.getRho(), moistAir.getX(), minMassFlowDa);
            }
            case DA_MASS_FLOW -> {
                return minMassFlowDa;
            }
            case MA_MASS_FLOW -> {
                return PhysicsOfFlow.calcMaMassFlowFromDaMassFlow(moistAir.getX(), minMassFlowDa);
            }
        }
        throw new FlowArgumentException("Invalid type of flow, cannot return value");
    }
    ...
    public void setMinFlow(TypeOfAirFlow typeOfFlow, double minFlow) {
        ...
        switch (typeOfFlow) {
            case DA_VOL_FLOW ->
                    this.minMassFlowDa = PhysicsOfFlow.calcDaMassFlowFromDaVolFlow(moistAir.getRho_Da(), minFlow);
            case MA_VOL_FLOW ->
                    this.minMassFlowDa = PhysicsOfFlow.calcDaMassFlowFromMaVolFlow(moistAir.getRho(), moistAir.getX(), minFlow);
            case DA_MASS_FLOW -> this.minMassFlowDa = minFlow;
            case MA_MASS_FLOW ->
                    this.minMassFlowDa = PhysicsOfFlow.calcDaMassFlowFromMaMassFlow(moistAir.getX(), minFlow);
        }
    }
    ...
    public static FlowOfMoistAir ofM3hVolFlow(double volFlowMaM3h, double tx, double RH, double Pat) {
        return FlowOfMoistAir.ofM3hVolFlow(Defaults.DEF_FLOW_NAME, volFlowMaM3h, tx, RH, Pat);
    }
