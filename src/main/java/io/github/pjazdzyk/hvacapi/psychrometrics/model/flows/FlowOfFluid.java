...
import io.github.pjazdzyk.hvacapi.psychrometrics.model.properties.LiquidWater;
...
public class FlowOfFluid implements Flow, Serializable {
    ...
    public void updateFlows() {
        Validators.validateForNotNull("locked flow", lockedTypeOfFluidFlow);
        switch (lockedTypeOfFluidFlow) {
            case MASS_FLOW -> volFlow = PhysicsOfFlow.calcVolFlowFromMassFlow(fluid.getRho(), massFlow);
            case VOL_FLOW -> massFlow = PhysicsOfFlow.calcMassFlowFromVolFlow(fluid.getRho(), volFlow);
        }
    }
    ...
    public static FlowOfFluid ofM3hWaterVolFlow(double volFlowMaM3h, double tx) {
        return FlowOfFluid.ofM3hWaterVolFlow(Defaults.DEF_FLOW_NAME, volFlowMaM3h, tx);
    }
